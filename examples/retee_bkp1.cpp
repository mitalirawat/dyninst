...
void loadlibraryWrappper(char*);

void loadlibraryWrappper(char* name){
DWORD  retval=0;
    BOOL   success; 
        TCHAR  buffer[BUFSIZE]=TEXT(""); 
	    TCHAR  buf[BUFSIZE]=TEXT(""); 
	        TCHAR** lppPart={NULL};
retval = GetFullPathName("C:\\Users\\mrawat\\dyninst-builds\\dyninst-release-install-64\\lib\\dyninstAPI_RT.dll",BUFSIZE,
                 buffer,lppPart);
if (retval == 0) 
    {
...
int main(int argc, char *argv[]) { 
    ...
    if (!app) return -1;
	fprintf(stderr, "process attached");
    // Read the program's image and get an associated image object 
    BPatch_image *appImage = app->getImage();
	fprintf(stderr, "got the app image"); 
    std::vector<BPatch_function*> writeFuncs; 
